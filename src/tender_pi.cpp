...
#include "tender_icons.xpm"

// the class factories, used to create and destroy instances of the PlugIn

extern "C" DECL_EXP opencpn_plugin* create_pi( void *ppimgr )
{
    return (opencpn_plugin *) new tender_pi( ppimgr );
}
...
tender_pi::~tender_pi( void )
{
}

int tender_pi::Init( void )
{
    AddLocaleCatalog( _T("opencpn-tender_pi") );
    m_config_version = -1;
...
int tender_pi::GetAPIVersionMajor()
{
    return MY_API_VERSION_MAJOR;
}

int tender_pi::GetAPIVersionMinor()
{
    return MY_API_VERSION_MINOR;
}

int tender_pi::GetPlugInVersionMajor()
{
    return PLUGIN_VERSION_MAJOR;
}

int tender_pi::GetPlugInVersionMinor()
{
    return PLUGIN_VERSION_MINOR;
}

wxBitmap *tender_pi::GetPlugInBitmap()
{
    return m_pplugin_icon;
}

wxString tender_pi::GetCommonName()
{
    return _("Tender");
}

wxString tender_pi::GetShortDescription()
{
    return _("Tender PlugIn for OpenCPN");
}

wxString tender_pi::GetLongDescription()
{
    return _("Tender PlugIn for OpenCPN");

}

void tender_pi::PopupMenuHandler( wxCommandEvent& event )
{
    bool handled = false;
    switch( event.GetId() ) {
        case ID_EPL_DELETE:
...
bool tender_pi::MouseEventHook( wxMouseEvent &event )
{
    bool bret = false;
        ...
        if( m_sel_brg || m_bshow_fix_hat){
            ...
            if(!m_bshow_fix_hat && m_sel_brg){
                ...
                GetOCPNCanvasWindow()->Connect( ID_EPL_DELETE, wxEVT_COMMAND_MENU_SELECTED,
                                                wxCommandEventHandler( tender_pi::PopupMenuHandler ), NULL, this );
            }
...
void tender_pi::setTrackedWPSelect(wxString GUID)
{
    if(GUID.Length()){
...
void tender_pi::ProcessTimerEvent( wxTimerEvent& event )
{
...
void tender_pi::OnRolloverPopupTimerEvent( wxTimerEvent& event )
{
...
void tender_pi::setIcon( char ** xpm_bits)
{
...
void tender_pi::ComputeShipScaleFactor(float icon_hdt,
...
void tender_pi::RenderIconDC(wxDC &dc )
{
    wxPoint ab;
...
bool tender_pi::RenderOverlay(wxDC &dc, PlugIn_ViewPort *vp)
{
    g_vp = vp;
...
bool tender_pi::RenderGLOverlay(wxGLContext *pcontext, PlugIn_ViewPort *vp)
{
    g_gdc = NULL;
    g_pdc = NULL;
...
void tender_pi::SetNMEASentence( wxString &sentence )
{
    m_NMEA0183 << sentence;
...
void tender_pi::SetPositionFix( PlugIn_Position_Fix &pfix )
{
    if(1){
...
void tender_pi::SetCursorLatLon( double lat, double lon )
{
}

void tender_pi::SetPluginMessage(wxString &message_id, wxString &message_body)
{
}

int tender_pi::GetToolbarToolCount( void )
{
    return 1;
}


void tender_pi::SetColorScheme( PI_ColorScheme cs )
{
}


void tender_pi::OnToolbarToolCallback( int id )
{
}


bool tender_pi::LoadConfig( void )
{
    ...
    if( pConf ) {
        ...
        pConf->SetPath ( _T( "/Settings/Tender_PI" ) );
...
bool tender_pi::SaveConfig( void )
{
    wxFileConfig *pConf = (wxFileConfig *) m_pconfig;
...
void tender_pi::ApplyConfig( void )
{

}


void tender_pi::ProcessBrgCapture(double brg_rel, double brg_subtended, double brg_TM, int brg_TM_flag,
...
int tender_pi::CalculateFix( void )
{
...
void tender_pi::RenderFixHat( void )
{
    if(!m_nfix)
        return;                 // no fix
...
void tender_pi::startSerial(const wxString &port)
{
...
void tender_pi::ShowPreferencesDialog( wxWindow* parent )
{
    TenderPrefsDialog *dialog = new TenderPrefsDialog( parent, wxID_ANY, _("TENDER_PI Preferences"), wxPoint( 20, 20), wxDefaultSize, wxDEFAULT_DIALOG_STYLE );
    dialog->Fit();
    wxColour cl;
...
void tender_pi::ProcessTenderFix( void )
{
...
PI_EventHandler::PI_EventHandler(tender_pi * parent)
{
    m_parent = parent;
